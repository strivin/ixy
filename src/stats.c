...
static double diff_pps(uint64_t pkts_new, uint64_t pkts_old, uint64_t nanos) {
	return (double) (pkts_new - pkts_old) / ((double) nanos / 1000000000.0);
}

static uint32_t diff_bit(uint64_t bytes_new, uint64_t bytes_old, uint64_t pkts_new, uint64_t pkts_old, uint64_t nanos) {
	...
	return (uint32_t) (((bytes_new - bytes_old) / ((double) nanos / 1000000000.0)) * 8
		+ diff_pps(pkts_new, pkts_old, nanos) * 20 * 8);
}

void print_stats_diff(struct device_stats* stats_new, struct device_stats* stats_old, uint64_t nanos) {
	printf("[%s] RX: %d bit/s %.2f pps\n", stats_new->device ? stats_new->device->pci_addr : "???",
		diff_bit(stats_new->rx_bytes, stats_old->rx_bytes, stats_new->rx_pkts, stats_old->rx_pkts, nanos),
		diff_pps(stats_new->rx_pkts, stats_old->rx_pkts, nanos)
	);
	printf("[%s] TX: %d bit/s %.2f pps\n", stats_new->device ? stats_new->device->pci_addr : "???",
		diff_bit(stats_new->tx_bytes, stats_old->tx_bytes, stats_new->tx_pkts, stats_old->tx_pkts, nanos),
		diff_pps(stats_new->tx_pkts, stats_old->tx_pkts, nanos)
	);
}
...
void print_stats_diff_m(struct device_stats* stats_new, struct device_stats* stats_old, uint64_t nanos) {
